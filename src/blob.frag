...
void main(){
    ...
    float value=perlinNoise(vec3(position*1.3,iTime*.17),seed);// single octave perlin noise
    ...
    float hue=mix(.36,1.,(perlinNoise(vec3(position,iTime*.1)*colorFrequency,hash(seed,0x0U))+1.)*.5);
    float saturation=mix(.6,.8,(perlinNoise(vec3(position,iTime*.1)*colorFrequency,hash(seed,0x5U))+1.)*.5);
    
    vec4 color=vec4(hsv2rgb(vec3(hue,saturation,.8)),value);
